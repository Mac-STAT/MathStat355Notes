...
\frac{1}{\sqrt{n}} \sqrt{n}(Y - \mu) & \overset{d}{\to} N(0, \sigma^2) \times 0 \\
 Y - \mu & \overset{d}{\to} 0
\end{align*}
and if a random variable (in this case $Y - \mu$) converges in distribution to a constant, that implies that it converges in probability to that same constant (see Wikipedia [here](https://en.wikipedia.org/wiki/Proofs_of_convergence_of_random_variables#propB1)). Then the CLT actually *implies* consistency of $Y$ for $\mu$. 
