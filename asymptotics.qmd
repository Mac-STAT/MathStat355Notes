...
$$
\frac{\overline{X} - \mu}{\sigma/\sqrt{n}} \overset{d}{\to} N(0, 1).
$$
...
$$
\Pr\left(-1.96 \leq \frac{\overline{X} - \mu}{\sigma/\sqrt{n}} \leq 1.96\right) = 0.95
$$

We can rearrange the probability statement on the left hand side to get

$$
\Pr\left(\overline{X}-1.96 \sigma/\sqrt{n} \leq \mu \leq \overline{X} + 1.96\sigma/\sqrt{n}\right) = 0.95
$$

and therefore, our 95% confidence interval for $\mu$ is given by $\left( \overline{X}-1.96 \sigma/\sqrt{n}, \overline{X}+1.96 \sigma/\sqrt{n} \right)$.
...
$$
Z_n = \frac{\sqrt{n}(\overline{X} - \mu)}{\sigma} = \frac{1}{\sqrt{n}} \sum_{i = 1}^n Y_i
$$
...
\begin{align*}
    ...
    & = \frac{1}{\sigma\sqrt{n}}  (n\overline{X} - n\mu) \\
...
We'll now make use of a theorem regarding sequences of real numbers (without proof): Let $a_n$ and $c_n$ be sequences or real numbers such that $a_n \overset{n \to \infty}{\to} 0$ and $c_na_n^2 \overset{n \to \infty}{\to} 0$. Then if $a_nc_n \overset{n \to \infty}{\to} b$, $(1 + a_n)^{c_n} \overset{n \to \infty}{\to} e^b$.
...
Applying Slutsky's theorem, noting that $\frac{1}{\hat{p}_{MLE}(1-\hat{p}_{MLE})} \overset{p}{\to} \frac{1}{p(1 - p)}$, we then have

$$
\sqrt{n}\left(\frac{\hat{p}_{MLE} - p}{\hat{p}_{MLE}(1-\hat{p}_{MLE})}\right) \overset{d}{\to} N(0, 1)
$$
...
```{=tex}
\begin{align*}
    0.95 & = \Pr(-1.96 < \frac{\hat{p} - p}{\hat{p}(1-\hat{p})}  < 1.96)  \\
    & = \Pr(-1.96\hat{p}(1-\hat{p}) < \hat{p} - p  < 1.96\hat{p}(1-\hat{p})) \\
    & = \Pr(\hat{p} -1.96\hat{p}(1-\hat{p}) <  p  < \hat{p} + 1.96\hat{p}(1-\hat{p}))
\end{align*}
```
**Problem 3:** Let $F_{m,n}$ denote a RV having the $F$-distribution with $m$ and $n$ degrees of freedom. Suppose $m = n$, and show that $\sqrt{n}(F_{n,n} - 1) \overset{d}{\to} N(0,4)$. Use the fact that if $X_n \overset{d}{\to} X$ and $h$ is a continuous function, then $h(X_n) \overset{d}{\to} h(X)$.
