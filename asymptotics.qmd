...
```{=tex}
\begin{align*}
    0.95 & = \Pr(-1.96 < \frac{\hat{p} - p}{\sqrt{n\hat{p}(1-\hat{p})}}  < 1.96)  \\
    & = \Pr(-1.96\sqrt{n\hat{p}(1-\hat{p})} < \hat{p} - p  < 1.96\sqrt{n\hat{p}(1-\hat{p})}) \\
    & = \Pr(\hat{p} -1.96\sqrt{n\hat{p}(1-\hat{p})} <  p  < \hat{p} + 1.96\sqrt{n\hat{p}(1-\hat{p})})
\end{align*}
```
