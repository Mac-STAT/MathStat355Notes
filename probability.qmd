...
\end{align*} 
$$where to obtain the last equality we use chain rule! Now we require some statistical trickery to continue\dots (note that this method is called the "CDF method" because we go *through* the CDF to derive the distribution for $Y$)
...
which is exactly the pdf of a geometric random variable with parameter $p$ and trials that begin at 1 (as opposed to 0), as makes sense with the Poisson distribution.
